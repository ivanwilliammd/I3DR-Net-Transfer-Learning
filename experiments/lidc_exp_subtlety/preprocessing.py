...
def pp_patient(inputs):
    ...
    print('\n\n     processing {}'.format(pid), img.GetSpacing(), img_arr.shape)
    ...
    for rid in roi_ids:
        ...
        rater_labels = [df[df.NoduleID == int(ii)].Texture.values[0] for ii in nodule_ids]
        print('rater_labels : '+str(rater_labels))

        # rater_labels.extend([0] * (4-len(rater_labels)))
        # print('rater_labels extended : '+str(rater_labels))

        texture_label = np.mean([ii for ii in rater_labels if ii > -1])
        print('texture_label : '+str(texture_label))
        ...
        roi_rater_list.extend([np.copy(roi_rater_list[-1])]*(4-len(roi_id_paths)))
        ...
        if np.sum(roi_raters) > 0:
            ...
            rix += 1
            print('texture_labels FINALSSSSSS: '+str(texture_labels))
            print('rix : '+str(rix))
        else:
            # indicate rois suppressed by majority voting of raters
            print('suppressed roi! texture category is less than 0', roi_id_paths)
    ...
    print('texture_labels : '+str(texture_labels))
    
    assert len(texture_labels) + 1 == len(np.unique(final_rois)), [len(texture_labels), np.unique(final_rois), pid]
    ...
    with open(os.path.join(cf.pp_dir, 'meta_info_{}.pickle'.format(pid)), 'wb') as handle:
        meta_info_dict = {'pid': pid, 'class_target': texture_labels, 'spacing': img.GetSpacing(), 'fg_slices': fg_slices}
