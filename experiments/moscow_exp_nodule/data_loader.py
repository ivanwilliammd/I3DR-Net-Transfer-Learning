...
# batch generator tools from https://github.com/ivanwilliammd/batchgenerators
...
def load_dataset(cf, logger, subset_ixs=None):
    ...
    for ix, pid in enumerate(pids):
        ...
        # for the experiment conducted here, texture scores are converted into 1 --> 1 = true_nodule
...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        # maybe error because at folds1, need one of each class --> 2 line dibawah ini
        ...
        # num_classes = self.cf.head_classes - 1
        class_targets=class_targets_list
        # np.random.seed(0)
        length = len(class_targets)
        for ix in range(batch_size):
            keep_looking = True
            while keep_looking:
                cand = np.random.choice(length, 1)[0]
                if (class_targets[cand].count(0) > 0):
                    keep_looking = False            
