...
class net(nn.Module):
    ...
    def forward(self, img):
        ...
        ### PRINT IMAGE INPUT
        # image_input = img.cpu().numpy()
        # shape_input = image_input.shape
        # print("summary to RetinaNet model using input from img feed = "+str(shape_input))
        
        #### STACK INPUT 3x into 3-dimensional tensor ####
        # img_list=[img,img,img]
        img_stack=torch.cat((img, img, img), 1)
        

        # Feature extraction
        # fpn_outs = self.Fpn(img)
        
        fpn_outs = self.Fpn(img_stack)
